...
## (후보) 다른 소재 — 필요하면 완성된 형태로 요청

- **Gemini의 확신에 찬 설명 검증 실패 사례**: 클래스 상속 순서(MRO)에 대해
  Gemini가 서로 모순되는 두 가지 설명을 자신 있게 제시했고, 실제로는 둘 다
  실행 결과와 공식 문서 기준으로 검증했을 때 "순서는 상관없다"가 정답이었던
  사례. AI 도구를 사용할 때의 검증 습관을 보여줄 수 있는 소재.
- **마이그레이션 도구 전환 중 발견한 서버 크래시**: `Base.metadata.create_all()`
  방식에서 Alembic으로 전환하는 과정에서, 정리가 안 된 채 남아있던 import
  구문 때문에 서버 기동 자체가 실패하는 문제를 실행 검증으로 미리 발견한 사례.
